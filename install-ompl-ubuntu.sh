...
install_app_dependencies()
{
    ...
    if ! pkg-config --atleast-version=0.5.0 fcl; then
        if [[ $ubuntu_version > 1604 ]]; then
            ...
            cd fcl-0.5.0; cmake .; -E make install; cd ..
        fi
    fi
}
...
install_ompl()
{
    ...
    wget -O - https://bitbucket.org/ompl/ompl/downloads/$OMPL-0.13.0-Source.tar.gz | tar zxf -
    cd $OMPL-0.13.0-Source
